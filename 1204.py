...
try:
    ...
    if ticker is not None:
        ...
        if bsheet.empty:
            ...
            pb_ratio_history = [
                bsheet.loc['Stockholders Equity', year] / (
                            income.loc['Net Income', year] / income.loc['Basic EPS', year])
                for year in years[::-1]]
            ...
            debt_equity_rank = sorted(debt_equity_list)
            ...
            pre_t1 = TTM_bsheet4['Receivables'] / income[years[1 - len(years)]][
                'Total Revenue'] if 'Receivables' in TTM_bsheet4 else 0
            ...
            pre_t5 = income[years[1 - len(years)]]['Selling General And Administration'] / \
                     income[years[1 - len(years)]]['Total Revenue']
            sgai = t5 / pre_t5
            # Depreciation Index
            t6 = TTM_cfs['Depreciation Amortization Depletion'] / (
                        TTM_cfs['Depreciation Amortization Depletion'] + TTM_bsheet[
                    'Net PPE']) if 'Depreciation Amortization Depletion' in TTM_cfs else 0
            pre_t6 = cfs[years[1 - len(years)]]['Depreciation Amortization Depletion'] / (
                        cfs[years[1 - len(years)]]['Depreciation Amortization Depletion'] + TTM_bsheet4[
                    'Net PPE']) if 'Depreciation Amortization Depletion' in cfs else 0
            depi = pre_t6 / t6 if t6 != 0 else 0
            # leverage Index
            t7 = (TTM_bsheet['Long Term Debt'] + TTM_bsheet['Current Liabilities']) / TTM_bsheet[
                'Total Assets'] if 'Long Term Debt' in TTM_bsheet else 0
            pre_t7 = (TTM_bsheet4['Long Term Debt'] + TTM_bsheet4['Current Liabilities']) / TTM_bsheet4[
                'Total Assets'] if 'Long Term Debt' in TTM_bsheet4 else 0
            lvgi = t7 / pre_t7 if pre_t7 != 0 else 0
            # Total Accruals to Total Assets
            tata = (TTM['Net Income Continuous Operations'] - TTM_cfs['Operating Cash Flow']) / TTM_bsheet[
                'Total Assets']
            ...
            summary, f_score, valuation, guru = st.tabs(
                ["Summary", "F-Score", "Valuation", "Financial Analysis"])
            ...
            with summary:
                ...
                if dataa.empty:
                    ...
                    fig.update_layout(autosize=True, width=1370, height=750,
                ...
                fig = px.bar(dfv, x, y=columns_to_plot, title='Financial Ratios',
                             ...
                             height=500, width=1100, barmode='group')
            ...
            with valuation:
                ...
                with col5:
                    ...
                    number5_str = st.text_input("Total Debt:", value=formatted_ttm_cr, placeholder="Type a number...")
            ...
            with guru:
                ...
                with col1:
                    ...
                    st.subheader('Score')
                    # WACC vs ROIC
                    risk_free_str = st.text_input('Risk Free Rate', value="%")
                    if risk_free_str.strip() == "%":
                        risk_free = 0.0
                    else:
                        risk_free = float(risk_free_str.replace(',', '').replace('%', '')) / 100

                    market_risk_str = st.text_input('Average Market Risk', value="%")
                    if market_risk_str.strip() == "%":  # Kiểm tra nếu chuỗi chỉ là ký tự '%'
                        market_risk = 0.0  # Gán giá trị mặc định cho trường hợp này
                    else:
                        market_risk = float(market_risk_str.replace(',', '').replace('%', '')) / 100

                    capm = risk_free + mck.info['beta'] * market_risk  # cost of equity
                    ...
                    wacc = (market_cap / (market_cap + ave_debt['Total Debt'])) * capm + (
                                ave_debt['Total Debt'] / (market_cap + ave_debt['Total Debt'])) * cost_of_debt * (
                                       1 - tax_rate)

                    # roic
                    invest_cap_dec = TTM_bsheet['Total Assets'] - TTM_bsheet['Payables And Accrued Expenses'] - (
                                TTM_bsheet['Cash Cash Equivalents And Short Term Investments']
                                - max(0, (TTM_bsheet['Current Liabilities'] - TTM_bsheet['Current Assets'] + TTM_bsheet[
                            'Cash Cash Equivalents And Short Term Investments']))) if 'Payables And Accrued Expenses' in TTM_bsheet else 0
                    invest_cap_sep = TTM_bsheet2['Total Assets'] - TTM_bsheet2['Payables And Accrued Expenses'] - (
                                TTM_bsheet2['Cash Cash Equivalents And Short Term Investments']
                                - max(0, (
                                    TTM_bsheet2['Current Liabilities'] - TTM_bsheet2['Current Assets'] + TTM_bsheet2[
                                'Cash Cash Equivalents And Short Term Investments']))) if 'Payables And Accrued Expenses' in TTM_bsheet2 else 0
                    roic = TTM['Operating Income'] * (1 - tax_rate) / (1 / 2 * (invest_cap_dec + invest_cap_sep)) if (
                                                                                                                                 1 / 2 * (
                                                                                                                                     invest_cap_dec + invest_cap_sep)) != 0 else 0

                    wacc_roic = wacc / roic if roic != 0 else None
                    data_score = pd.DataFrame(
                        {
                            "STT": [1, 2, 3, 4],
                            "Index": ['WACC vs ROIC', 'Altman Z-Score', 'Beneish M-Score',
                                      'Piotroski F-Score (Scale of 9)'],
                ...
                with col2:
                    ...
                    st.subheader('GF Values: ' + str(gfvalues_score) + '/' + '10')
                    data_GF_Value = pd.DataFrame(
                        {
                            ...
                            "Index": ['PE Ratio', 'PS Ratio', 'PB Ratio', 'Price-to-tangible-book Ratio',
                                      ...
                                      'FCF Yield %'],
                            "-": [PE_ratio_list, PS_ratio_list, PB_ratio_list, Price_to_TBV_list, price_to_FCF_list,
                                  EV_to_EBIT_list,
                                  EV_to_EBITDA_list, EV_to_Revenue_list, EV_to_FCF_list, Price_to_Net_CAV_list,
                                  EarningsYields_list,
                                  FCFYield_list],
                            ...
                            "Vs History": [PE_ratio_values, PS_ratio_values, PB_ratio_values, Price_to_TBV_values,
                                           price_to_FCF_values, EV_to_EBIT_values,
                                           EV_to_EBITDA_values, EV_to_Revenue_values, EV_to_FCF_values,
                                           Price_to_Net_CAV_values, EarningsYields_values,
                                           FCFYield_values],
                        },
                    )
                    st.data_editor(
                ...
                with col10:
                    ...
                    for col in columns_to_plot4:
                        ...
                        fig.update_traces(text=new_values.apply(lambda x: f'{x:.2f}B'), textposition='outside',
                                          selector=dict(name=col))
                    fig.update_xaxes(fixedrange=True, title_text="Time")
                    fig.update_layout(legend=dict(orientation='h', x=0.5, y=1.35), width=600, height=400)
